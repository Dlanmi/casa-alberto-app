  ...
  async function handlePDF(): Promise<void> {
    ...
    try {
      ...
      const result = (await window.api.pdf.generarFactura({
        ...
        items: pdfItems,
        subtotal: pedidoData?.subtotal ?? factura.total,
        totalMateriales: pedidoData?.totalMateriales ?? 0,
        precioLista: pedidoData?.precioLista ?? factura.total,
        descuentoMonto: pedidoData?.descuentoMonto ?? 0,
        descuentoMotivo: pedidoData?.descuentoMotivo ?? null,
        total: factura.total,
        pagos: pdfPagos,
