  ...
  return (
    <header className="h-14 flex items-center justify-between px-6 border-b border-border bg-surface shrink-0">
      ...
      <button
        ...
        className="flex items-center gap-2 h-9 px-3 rounded-md border border-border bg-surface text-text-muted hover:border-border-strong hover:text-text transition-colors cursor-pointer"
        aria-label="Abrir búsqueda global"
      >
        <Search size={16} />
        <span className="text-sm">Buscar…</span>
        <kbd className="hidden sm:inline-block text-xs bg-surface-muted px-1.5 py-0.5 rounded text-text-soft">
          {formatPrimaryShortcut('k')}
        </kbd>
      </button>

      <div className="flex items-center gap-4">
        <div className="hidden xl:flex items-center gap-2 rounded-full bg-surface-muted px-3 py-1.5 text-xs text-text-muted">
          <Sparkles size={14} className="text-accent-strong" />
          <span>{getRouteHint(location.pathname)}</span>
        </div>

        <div className="text-right hidden md:block">
          <p className="text-sm font-medium text-text">{getGreeting()}, Alberto</p>
          <p className="text-[11px] text-text-muted">{getFormattedDate()}</p>
        </div>
